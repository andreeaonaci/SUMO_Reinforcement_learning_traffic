...
class TrafficSignal:
    ...
    def set_next_phase(self, new_phase: int):
        ...
        Args:
            new_phase (int or torch.Tensor): Number between [0 ... num_green_phases].
        """
        if isinstance(new_phase, torch.Tensor):
            new_phase = new_phase.detach()
            if new_phase.numel() != 1:
                raise ValueError("new_phase must be a scalar.")
            new_phase = int(new_phase.item())
        else:
    ...
    def compute_observation(self):
        ...
        observation = self.observation_fn()
        if not isinstance(observation, np.ndarray):
            raise ValueError("Observation function must return a numpy array.")
        if observation.ndim != 1:
            raise ValueError("Observation must be a 1-dimensional numeric vector.")
        return observation

    def compute_reward(self) -> float:
        """Computes the reward of the traffic signal and ensures it is a scalar value."""
        if self.reward_dim == 1:
            ...
            if self.reward_weights is not None:
                self.last_reward = float(np.dot(self.last_reward, self.reward_weights))  # Linear combination of rewards

        if not isinstance(self.last_reward, (int, float)):
