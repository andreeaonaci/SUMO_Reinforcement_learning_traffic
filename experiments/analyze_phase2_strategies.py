...
BATCH_LOGS = [
    ...
    "results/phase2_7city_strategy_comparison_resume5.log",
]
...
STARTING_RESUME_RE = re.compile(
    r"starting (?P<tag>\S+) base_dir=\S+ flags='[^']*--resume (?P<run_dir>\S+)"
)
...
def discover_completed_runs() -> Dict[str, str]:
    ...
    A `--resume <run_dir>` job's "finished" line has an empty run_dir field
    (run_concurrent_batch.sh's `ls results/run_*_$PYPID` capture only works
    for freshly-created run_dirs, not ones reused via --resume) -- fall back
    to the run_dir named in that same tag's "starting" line.
    """
    tag_to_dir: Dict[str, str] = {}
    tag_to_resume_dir: Dict[str, str] = {}
    for rel_path in BATCH_LOGS:
        ...
        with open(path, errors="replace") as f:
            for line in f:
                m = STARTING_RESUME_RE.search(line)
                if m:
                    tag_to_resume_dir[m.group("tag")] = m.group("run_dir")
                    continue
                m = FINISHED_RE.search(line)
                if m and m.group("exit") == "0":
                    run_dir = m.group("run_dir") or tag_to_resume_dir.get(m.group("tag"), "")
                    if run_dir:
                        tag_to_dir[m.group("tag")] = run_dir
    return tag_to_dir
...
def main() -> None:
    ...
    section_nums = [int(n) for n in re.findall(r"^## (\d+)\.", existing_content, re.MULTILINE)]
    next_section = max(section_nums, default=0) + 1

    lines = []
    lines.append(
        f"## {next_section}. Phase 2 aggregation-strategy comparison, 7-city, auto-generated {today}\n"
    )
    lines.append(
    ...
    updated = existing_content[:idx] + new_section + "\n" + existing_content[idx:]
    with open(FINDINGS_PATH, "w") as f:
        f.write(updated)
    print(f"\nAppended §{next_section} to {FINDINGS_PATH}")
